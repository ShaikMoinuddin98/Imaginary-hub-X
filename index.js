...
passport.use(new LocalStrategy({ usernameField: 'email',passwordField: 'pass' }, data.authenticate()));
...
const sendWeeklyTopArticles = async () => {
  ...
  for (let user of subs) {
      ...
      if (topArticles.length > 0) {
        ejs.renderFile(__dirname +"/views/template.ejs", { articles: topArticles }, (err, html) => {
          if (err) {
...
app.get("/",async (req,res)=>{
  let arts=await articles.find({
    publishedDate: {$exists:true }}).sort({ engagementRatio: -1 }).limit(4);
  ...
  res.render("index.ejs",{data:arts})
})
...
app.get("/dashboard", isLoggedIn, async (req, res) => {
  ...
  if (!c[0].profession)
    ...
    res.render("dashboard.ejs", { data: c[0],articles:arts })
});
...
app.get("/about", async (req, res) => {
 ...
 let subs=await subscribers.countDocuments({})
  res.render("about.ejs",{views:totalviews,subs:subs,totalarts:arts.length})
})


app.get("/allnews", async (req, res) => {
  let arts = await articles.find({ publishedDate: { $exists: true } }).sort({ engagementRatio: -1 })
...
app.get("/article/:title/:id", async (req, res) => {
  ...
  let a=await articles.findOne({title:req.params.title,_id:req.params.id})
  let u=await creator.findOne({email:a.email},"username")
  let top3arts=await articles.find({ 
    publishedDate: { $exists: true },email:a.email 
}).sort({ engagementRatio: -1 }).limit(3);

  if(a.publishedDate)
  res.render("newstemplate.ejs",{data:a,toparts:top3arts,username:u.username})
  else
  res.json({message:"Article Not yet published"})
})
...
app.post("/profile-creation", isLoggedIn, upload.single('profilePhoto'), async (req, res) => {
  try {
    ...
    if (req.file) {
      updata = {
        username: req.body.username, profession: req.body.profession, socialmedia: l, description: req.body.profileDescription, img: {
          data: req.file.buffer,
          contentType: req.file.mimetype
        }, primarySocial: req.body.primarySocial, heading: req.body.profileHeading, interests: interestarr
      }
    }
    else {
      updata = { username: req.body.username, profession: req.body.profession, socialmedia: l, description: req.body.profileDescription, primarySocial: req.body.primarySocial, heading: req.body.profileHeading, interests: interestarr }
    }
...
app.get("/profile/:name", async(req, res) => {
  try {
    ...
    if (d) {
      res.render("profile-preview.ejs", { data: d,engagement:totalengagement.toFixed(0),views:totalviews,totalarticles:arts.length,joined:d._id.getTimestamp() });
    } else {
...
app.get("/get-analytics",isLoggedIn,async (req,res)=>{
  let article=await articles.find({email:req.user.email},"title views shares timespent scrolldepth engagementRatio")
